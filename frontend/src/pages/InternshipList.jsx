  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!cancelled) {
          setError(getErrorMessage(e, 'Failed to load internships'));
          setData(null);
        }
      } finally {
  ...
  return (
    <div>
      <div className="page-header d-flex flex-wrap justify-content-between align-items-start gap-3">
        ...
        {isAdmin && (
          <Link to="/internships/add" className="btn btn-primary">
            Add Internship
          </Link>
        )}
      </div>
              ...
              {items.length === 0 ? (
                ...
                items.map((row) => (
                  <tr key={row.id}>
                    ...
                    <td>{fmtStipend(row.stipend)}</td>
                    <td>
                      <StatusBadge status={row.status} />
                    </td>
                    <td>
                      <div className="d-flex flex-wrap gap-2">
                        <Link to={`/internships/${row.id}`} className="btn btn-sm btn-outline-secondary">
                          View
                        </Link>
                        {isAdmin && (
                          <Link to={`/internships/${row.id}/edit`} className="btn btn-sm btn-outline-primary">
                            Edit
                          </Link>
                        )}
                        {isAdmin && (
